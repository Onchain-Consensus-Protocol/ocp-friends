...
function VaultCard({ vault, zh, onNavigate }: { key?: React.Key; vault: MyVault; zh: boolean; onNavigate: (href: string) => void }) {
  ...
  return <a href={href} onClick={(event) => { event.preventDefault(); onNavigate(href); }} className="friends-card group rounded-2xl border border-fuchsia-400/20 bg-[#0d0618]/80 p-5 hover:border-fuchsia-400/50">
    ...
    <div className="mt-4 flex flex-wrap gap-2 text-[10px] text-text-muted"><span className="rounded-md border border-border px-2 py-1">{vault.mode === 0 ? "AUTOMATIC_MAJORITY" : (zh ? "创建者结算" : "Creator Resolved")}</span>{vault.outcome > 0 && <span className="rounded-md border border-border px-2 py-1">{zh ? "结果" : "Result"}: {outcome}</span>}</div>
