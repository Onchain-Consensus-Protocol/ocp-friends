...
export function CreatePrivateVault({ lang, wallet, onNavigate }: { lang: Language; wallet: WalletController; onNavigate: (href: string) => void }) {
  ...
  const create = async () => {
    ...
    if (config.privateVaultFactoryAddress === ZERO) return setError(zh ? "Market 工厂尚未配置。" : "The Market factory is not configured.");
  ...
  return <main className="relative mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <div className="friends-card space-y-6 rounded-3xl border border-fuchsia-400/20 bg-[#120921]/90 p-6 shadow-xl backdrop-blur sm:p-8">
        ...
        <Button onClick={create} disabled={busy || Boolean(invalid.length)} variant="primary" className="w-full justify-center !rounded-xl !bg-gradient-to-r !from-[#ff7628] !via-[#ff3cac] !to-[#8257f5] !py-3.5 shadow-[0_0_24px_rgba(255,118,40,0.24)]">{busy ? (zh ? "正在创建…" : "Creating…") : wallet.connected ? <>{zh ? "创建 " : "Create "}<FriendsBrand /> Market</> : (zh ? "连接钱包后创建" : "Connect Wallet to Create")}</Button>
      </div>
  </main>;
}
