  ...
  return <div className="friends-page min-h-screen text-text">
    ...
    <main className="relative mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <div className="friends-card space-y-6 rounded-3xl border border-fuchsia-400/20 bg-[#120921]/90 p-6 shadow-xl backdrop-blur sm:p-8">
        <Field label={zh ? "主题" : "Claim"}><input aria-label="Claim" value={claim} onChange={(e) => setClaim(e.target.value)} className="input" placeholder={zh ? "这次要预测什么？" : "What will be resolved?"} /></Field>
        <Field label={zh ? "说明" : "Description"}><textarea aria-label="Description" value={description} onChange={(e) => setDescription(e.target.value)} className="input min-h-28" placeholder={zh ? "把 YES、NO 和 INVALID 的含义说清楚。" : "Define YES, NO, and INVALID clearly."} /></Field>
