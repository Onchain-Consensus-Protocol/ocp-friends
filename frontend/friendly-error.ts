...
export function friendlyError(reason: unknown, zh: boolean): string {
  ...
  if (match("position is locked to one side")) return zh
    ? "你已经选择了另一个选项。同一个 Vault 中不能更换选项，只能继续增加原选项的金额。"
    : "You already selected another option in this Vault. You cannot switch sides, but you can add to your original choice.";
  ...
  return zh
    ? "操作失败。请确认钱包网络、余额和当前 Vault 状态后重试。"
    : "The action could not be completed. Check your wallet network, balance, and the current Vault status, then try again.";
}
