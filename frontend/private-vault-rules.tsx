...
const COPY = {
  en: {
    create: "Create Private Market", eyebrow: "FOR YOU & YOUR FRIENDS", title: "How to Play",
    intro: "Two ways to play. Two ways to settle.",
  ...
  zh: {
    create: "创建私人市场", eyebrow: "只属于你和朋友们", title: "玩法",
    intro: "2种玩法，2种结算方式",
    coreSub: "资金多数决定结果", creatorSub: "由创建者提交最终结果", step1: "第一步 · 判定结果",
...
export function RulesPage({ lang }: { lang: Language }) {
  const [mode, setMode] = useState<Mode>(initialMode);
  const t = COPY[lang];
  return <main className="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
      ...
      <div className="mb-8 grid gap-3 sm:grid-cols-2"><ModeButton active={mode === 0} onClick={() => setMode(0)} title={lang === "zh" ? "资金多数" : "AUTOMATIC_MAJORITY"} subtitle={t.coreSub} /><ModeButton active={mode === 1} onClick={() => setMode(1)} title={lang === "zh" ? "创建者结算" : "Creator Resolved"} subtitle={t.creatorSub} warning /></div>
