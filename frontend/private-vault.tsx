  ...
  return <main className="relative max-w-6xl mx-auto px-4 py-8 sm:px-6">
      ...
      {!state ? (!error && <Message text={zh ? "正在加载私人 Market…" : "Loading private Market…"} />) : <>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <section className="self-start border border-border rounded-2xl p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-display font-bold">{zh ? "你的权限" : "Your Access"}</h2>{state.allowed && <span className="rounded-full bg-success/10 px-2 py-1 text-[10px] font-bold text-success">{zh ? "已受邀" : "INVITED"}</span>}</div>{!wallet.connected ? <Button onClick={wallet.connectWallet} variant="outline" className="mt-3">{zh ? "连接钱包" : "Connect wallet"}</Button> : state.allowed ? <div className="mt-3 flex items-center gap-2 text-sm text-success"><ShieldCheck className="h-4 w-4" /><strong>{zh ? "可以参与这个 Market" : "You can join this Market"}</strong></div> : <div className="mt-3 text-danger text-sm"><strong>{zh ? "这是私人 Market。" : "This is a private Market."}</strong><p className="mt-1">{zh ? "当前钱包不在参与名单中。" : "Your wallet is not on the participant list."}</p></div>}<div className="mt-3 text-[11px] font-mono text-text-muted">{zh ? "仓位 " : "Position "}YES {fmt(state.userYes)} · NO {fmt(state.userNo)}</div><div className="mt-4 border-t border-border/70 pt-3"><div className="mb-2 flex items-center justify-between gap-3"><h3 className="text-[11px] font-bold uppercase tracking-wide text-text-muted">{zh ? "受邀地址" : "Invited addresses"}</h3><span className="rounded-full bg-white/5 px-2 py-0.5 text-[10px] font-mono text-text-muted">{inviteListLoading ? "…" : invitedWallets.length}</span></div>{!inviteListLoading && invitedWallets.length === 0 ? <p className="text-[11px] text-text-muted">{zh ? "暂时无法读取邀请名单。" : "The invite list is temporarily unavailable."}</p> : <div className="max-h-28 space-y-1 overflow-y-auto overscroll-contain pr-1 [scrollbar-gutter:stable]">{invitedWallets.map((address) => <div key={address} title={address} className="flex items-center gap-2 rounded-lg bg-white/[0.035] px-2.5 py-1.5 font-mono text-[11px] leading-4 text-text-muted"><span className="min-w-0 flex-1 whitespace-nowrap">{short(address)}</span><span className="flex shrink-0 items-center gap-1">{address.toLowerCase() === state.creator.toLowerCase() && <span className="rounded-full bg-fuchsia-400/10 px-1.5 py-0.5 text-[9px] text-fuchsia-300">{zh ? "创建者" : "Creator"}</span>}{address.toLowerCase() === wallet.address?.toLowerCase() && <span className="rounded-full bg-success/10 px-1.5 py-0.5 text-[9px] text-success">{zh ? "你" : "You"}</span>}</span></div>)}</div>}</div></section>
        </div>
