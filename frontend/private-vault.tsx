  ...
  return <main className="relative max-w-6xl mx-auto px-4 py-8 sm:px-6">
      ...
      {!state ? (!error && <Message text={zh ? "正在加载私人 Market…" : "Loading private Market…"} />) : <>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <section className="self-start border border-border rounded-2xl p-5"><h2 className="font-display font-bold mb-3">{zh ? "你的权限" : "Your Access"}</h2>{!wallet.connected ? <Button onClick={wallet.connectWallet} variant="outline">{zh ? "连接钱包" : "Connect wallet"}</Button> : state.allowed ? <div className="text-success flex gap-2 items-center text-sm"><ShieldCheck className="w-4 h-4" /><strong>{zh ? "你已受邀。" : "You are invited."}</strong></div> : <div className="text-danger text-sm"><strong>{zh ? "这是私人 Market。" : "This is a private Market."}</strong><p className="mt-1">{zh ? "当前钱包不在参与名单中。" : "Your wallet is not on the participant list."}</p></div>}<div className="mt-3 text-[11px] font-mono text-text-muted">{zh ? "你的仓位：" : "Your position: "}YES {fmt(state.userYes)} · NO {fmt(state.userNo)}</div><div className="mt-4 border-t border-border pt-3"><div className="mb-2 flex items-center justify-between gap-3"><h3 className="text-[11px] font-bold uppercase tracking-wide">{zh ? "受邀地址" : "Invited addresses"}</h3><span className="text-[11px] font-mono text-text-muted">{inviteListLoading ? (zh ? "读取中…" : "Loading…") : invitedWallets.length}</span></div>{!inviteListLoading && invitedWallets.length === 0 ? <p className="text-[11px] text-text-muted">{zh ? "暂时无法读取邀请名单。" : "The invite list is temporarily unavailable."}</p> : <div className="max-h-28 space-y-1 overflow-y-auto overscroll-contain pr-1 [scrollbar-gutter:stable]">{invitedWallets.map((address) => <div key={address} title={address} className="flex items-center gap-1 rounded-md border border-border/60 bg-black/10 px-2 py-1 font-mono text-[10px] leading-4 text-text-muted"><span className="min-w-0 flex-1 truncate">{address}</span>{address.toLowerCase() === state.creator.toLowerCase() && <span className="whitespace-nowrap text-fuchsia-300">{zh ? "创建者" : "Creator"}</span>}{address.toLowerCase() === wallet.address?.toLowerCase() && <span className="whitespace-nowrap text-success">{zh ? "你" : "You"}</span>}</div>)}</div>}</div></section>
        </div>
